...
const fs=require('fs'),assert=require('assert/strict'),dir='audit-visual/map-polished';
...
(async()=>{
 ...
 for(const [name,width,height] of [['desktop',1440,1000],['mobile',375,812],['tablet',768,1024]]){
  ...
  report.push({name,...metrics,copyPreserved:true,errors,failed});console.log(JSON.stringify(report.at(-1)));await p.close();
 }
